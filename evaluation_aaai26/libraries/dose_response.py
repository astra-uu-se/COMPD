...
def plate_curves_after_error(layout_dir,layout_file,plate_content,expected_noise,error_function,error,normalization_function,min_dist=0,lose_from_row=0,lose_to_row=0, df_params=None,plate_type=None,compounds=None,concentrations=None,replicates=None):
...
def __run_experiment(layout_dir,layout_file,plate_content,expected_noise,error_function,error,normalization_function,min_dist,lose_from_row,lose_to_row,
                     plate_type=None,compounds=None,concentrations=None,replicates=None):

    layout = np.load(layout_dir+layout_file)
    if plate_type == None:
        None
    elif plate_type['type'] == 'COMPD':
...
# for dose-repsonse experiments, COMPD layouts use a different encoding compared to PLAID and random layouts
# thusly we need to update them before proceedign further
def update_compd_layout(plate_old, compounds, concentrations, replicates):
    ...
    list_coordinates = [[] for i in range(compounds*concentrations)]
    ...
    for i in range(compounds*concentrations):
        for j in range(len(list_coordinates[i])):
            (r,c) = list_coordinates[i][j]
